...
def _get_period_range(end_dt: datetime, range_type: str) -> tuple[datetime, datetime]:
    ...
    if range_type == "M":
        ...
        return start, end_dt
    if range_type == "Y":
        start = end_dt.replace(month=1, day=1, hour=0, minute=0, second=0, microsecond=0)
        return start, end_dt
    if range_type == "ALL":
...
def _filter_by_period(df: pd.DataFrame, start: datetime, end: datetime) -> pd.DataFrame:
...
def main_page(date_time_str: str, df: pd.DataFrame, settings: dict[str, Any]) -> str:
...
def _get_top_transactions(expenses: pd.DataFrame, n: int = 5) -> list[dict[str, Any]]:
...
def _get_expenses_block(expenses: pd.DataFrame) -> dict[str, Any]:
    ...
    # Суммы по категориям
    cat_totals = expenses.groupby("Категория")["Сумма платежа"].sum().round().astype(int)
    ...
    transfers_and_cash = [{"category": cat, "amount": int(amount)} for cat, amount in transfers.items()]
    ...
    main = [{"category": cat, "amount": int(amount)} for cat, amount in top7.items()]
...
def _get_income_block(income: pd.DataFrame) -> dict[str, Any]:
    ...
    cat_totals = income.groupby("Категория")["Сумма платежа"].sum().round().astype(int)
    cat_totals = cat_totals.sort_values(ascending=False)

    main = [{"category": cat, "amount": int(amount)} for cat, amount in cat_totals.items()]
